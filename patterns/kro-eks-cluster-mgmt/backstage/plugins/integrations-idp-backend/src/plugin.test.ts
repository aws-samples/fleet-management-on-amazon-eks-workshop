...
import { integrationsIdpPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [integrationsIdpPlugin],
    });

    await request(server).get('/api/integrations-idp/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/integrations-idp/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/integrations-idp/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/integrations-idp/todos/${createdTodoItem.id}`)
